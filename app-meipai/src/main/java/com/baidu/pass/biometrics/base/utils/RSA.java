package com.baidu.pass.biometrics.base.utils;

import android.text.TextUtils;
import e.c.b.e.Utils;
import java.io.ByteArrayInputStream;
import java.security.PublicKey;
import javax.crypto.Cipher;
import javax.security.cert.X509Certificate;

public class RSA {
    public static String RSA_PUBLIC_KEY = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----";
    public static String SO_UPDATE_PUBLIC_KEY_STR = "";

    public static String decrypt(String str) throws Exception {
        PublicKey publicKey = getPublicKey(SO_UPDATE_PUBLIC_KEY_STR);
        Cipher instance = Cipher.getInstance("RSA/ECB/PKCS1Padding");
        instance.init(2, publicKey);
        String str2 = "UTF-8";
        return new String(instance.doFinal(Utils.a(str.getBytes(str2))), str2).trim();
    }

    public static byte[] encrypt(String str) {
        return encrypt(str, RSA_PUBLIC_KEY);
    }

    public static PublicKey getPublicKey(String str) throws Exception {
        return X509Certificate.getInstance(new ByteArrayInputStream(Utils.a(str.getBytes()))).getPublicKey();
    }

    public static byte[] encrypt(String str, String str2) {
        if (str == null || TextUtils.isEmpty(str2)) {
            return new byte[0];
        }
        try {
            Cipher instance = Cipher.getInstance("RSA/NONE/PKCS1Padding");
            instance.init(1, X509Certificate.getInstance(new ByteArrayInputStream(str2.getBytes())).getPublicKey());
            return instance.doFinal(str.getBytes("UTF-8"));
        } catch (Throwable unused) {
            return new byte[0];
        }
    }
}
